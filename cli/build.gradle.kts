...
kotlin {
    ...
    sourceSets {
        ...
        // Native tests need the embedded TAR files on the test classpath so the generated
        // BblpacksResources loader (generated by generateTarResourceReaderKt) can load
        // "/embedded/tar/<translation>.tar".
        linuxX64Test.get().resources.srcDir(layout.buildDirectory.dir("embedded"))
    }
...
// Ensure Kotlin compilation sees the generated files. (These tasks depend on tarBblpacks and will
// generate GeneratedTarBindings.kt).
listOf(
    "compileKotlinLinuxX64" to listOf(":cli:generateTarBindingsKt"),
    "compileKotlinMacosX64" to listOf(":cli:generateTarBindingsKt"),
    "compileKotlinMacosArm64" to listOf(":cli:generateTarBindingsKt")
).forEach { (tn, deps) ->
    tasks.matching { it.name == tn }.configureEach {
        if (bblpacksEmbedEnabled.get()) {
            deps.forEach { dep -> dependsOn(dep) }
        }
    }
}
