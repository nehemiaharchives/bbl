describe 'bbl pack install and uninstall' do
  it 'uninstalls and reinstalls kjv' do
    bbl_cmd = "C:\\Users\\joel\\AppData\\Local\\Programs\\bbl\\bbl.exe"

    # Debug: check environment and packs directory in powershell resource
    env_check = powershell("$env:USERPROFILE = $env:USERPROFILE; Write-Output \"USERPROFILE=$env:USERPROFILE\"; Write-Output \"Packs dir: $(ls $env:USERPROFILE\\.bbl\\packs)\"; & '#{bbl_cmd}' list translations")
    ...
    script_content = "$env:BBL_PACK_BASE_URL = 'file://#{$bbl_install_source_dir}'; $env:USERPROFILE = $env:USERPROFILE; & '#{bbl_cmd}' install kjv"
    ...
    sleep 2
    after_install_list = powershell("$env:USERPROFILE = $env:USERPROFILE; Write-Output \"Packs dir after install: $(ls $env:USERPROFILE\\.bbl\\packs)\"; & '#{bbl_cmd}' list translations")
